...
import { ComponentsPicker } from "../components/ComponentsPicker/index.js";

describe("ComponentsPicker", () => {
  it("uses proper classes when lockedPicker is true", () => {
    const wrapper = shallow(
      <ComponentsPicker lockedPicker={true} onComponentClick={() => {}} />
    );
    ...
    // console.log(
    //   component.children().at(0).hasClass("components-picker__component")
    // );
  ...
  it("calls onComponentClick on click when lockedPicker is false", () => {
    ...
    component.children().at(0).simulate("click");

    expect(onComponentClickMock.mock.calls.length).toEqual(1);
  });
  ...
  it("does not call onComponentClick on click when lockedPicker is true", () => {
    ...
    const component = shallow(
      <ComponentsPicker lockedPicker onComponentClick={onComponentClickMock} />
    );

    component.children().at(0).simulate("click");

    expect(onComponentClickMock.mock.calls.length).toEqual(0);
  });
});
